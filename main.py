...
def register():
    ...
    if user_form.validate_on_submit():
        if (User.query.filter_by(email=user_form.email.data).first()):
            print(User.query.filter_by(email=user_form.email.data).first())
            flash("You've already signed up with that email. Please log in instead")
            return redirect(url_for('login'))

        hashed_pw = generate_password_hash(user_form.password.data, method='pbkdf2:sha256', salt_length=8)
        new_user = User(
